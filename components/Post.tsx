...
export default function Post({post}:PostProps) {
  ...
  return (
    ...
    <View style={styles.postHeader}>
        <Link href={ currentUser?._id === post.author._id ? "/(tabs)/profile" : `/user/${post.author._id}` }
        asChild>
        ...
        {/* delete post/if owner is verified */}

        {post.author._id === currentUser?._id ? (
        <TouchableOpacity onPress={handleDelete} ><Ionicons name="trash-outline" size={20} color={COLORS.primary} right={0} /></TouchableOpacity>
        ) : (
        <TouchableOpacity><Ionicons name="ellipsis-horizontal" size={20} right={17} color={COLORS.white} /></TouchableOpacity>
    )}
        </View>
   ...
   <View style={styles.postActions}>
    ...
    <TouchableOpacity onPress={handleBookmark}>
        <Ionicons name={isBookmarked ? "bookmark" : "bookmark-outline"} size={25} color={COLORS.white} />
    </TouchableOpacity>
   </View>
   {/*post info */}
