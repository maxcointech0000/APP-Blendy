...
import { View, Text, TouchableOpacity, ScrollView, FlatList, Modal, KeyboardAvoidingView, Keyboard, Platform, TextInput, TouchableWithoutFeedback } from "react-native";


export default function Profile() {
const { signOut, userId} = useAuth();
const [isEditModalVisible, setIsEditModalVisible] = useState(false);
const currentUser = useQuery(api.users.getuserByClerkId, userId ? {
  clerkId: userId } : "skip"
);
  ...
  ListHeaderComponent={
    <>
      ...
      <View style={styles.profileInfo}>
        ...
        <Text style={[styles.name, { bottom: 6 }]}>{currentUser.fullname}</Text>
        {currentUser.bio && <Text style={styles.bio}>{currentUser.bio}</Text>}
        <View style={styles.actionButtons}>
          <TouchableOpacity onPress={() => setIsEditModalVisible(true)} style={styles.editButton}>
            <Text style={styles.editButtonText}>Edit profile</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => signOut()} style={styles.shareButton}>
...
<Ionicons name="close" size={30} color={COLORS.white} />
          ...
          <View style={styles.inputContainer}>
           ...
           <TextInput 
           style={styles.input} value={editedProfile.fullname} 
          ...
          <View style={styles.inputContainer}>
           ...
           <TextInput 
           style={styles.input} value={editedProfile.bio} 
