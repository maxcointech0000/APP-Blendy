...
export const createPost = mutation({
    ...
    handler: async (ctx, args) => {
      ...
      const postId =  await ctx.db.insert("posts", {
        ...
        //increment user's post count by
...
//enhance pots with userdata and interaction status
       
        const postsWithInfo = await Promise.all(posts.map(async(post) => { const postAuthor = (await ctx.db.get(post.userId))!
...
export const deletePost = mutation({ args: {postId:v.id("posts")},
    handler: async (ctx, args) => {
        const currentUser = await getAuthenticatedUser(ctx);
...
for (const notification of notifications) {
    ...
    //decrement user's post cunt by 1
    await ctx.db.patch(currentUser._id,{
...
export const getPostByUser = query({args: {
